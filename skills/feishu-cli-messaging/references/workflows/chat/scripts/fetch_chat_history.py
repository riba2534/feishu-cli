...
def _find_repo_root(script_path: Path) -> Path | None:
    """从脚本位置向上寻找 feishu-cli 仓库，避免依赖易碎的固定目录层级。"""
    module_line = "module github.com/riba2534/feishu-cli"
    for parent in script_path.resolve().parents:
        go_mod = parent / "go.mod"
        try:
            if go_mod.is_file() and module_line in go_mod.read_text(encoding="utf-8").splitlines():
                return parent
        except OSError:
            continue
    return None
...
def find_cli(explicit: str | None) -> str:
    """定位 feishu-cli 二进制。优先级：用户传入 → 仓库内最新产物 → PATH。

    把仓库产物放在 PATH 之前是关键：本 skill 在飞书 CLI 仓库内开发时，应使用刚编译的
    当前源码版本，而 PATH 上可能是更老的 go install 安装版（实战中遇到过
    旧版不会自动加载 ~/.feishu-cli/token.json，导致脚本调用回落 App Token 失败）。
    仓库内同时支持 ``go build -o feishu-cli .`` 和 ``make build`` 产生的
    ``bin/feishu-cli``，两者都存在时选修改时间较新的可执行文件。脚本被装到
    ~/.claude/skills/ 等仓库外位置时不会猜测固定层级，安全回退到 PATH。"""
    if explicit:
        return explicit
    repo_root = _find_repo_root(Path(__file__))
    if repo_root:
        candidates = [repo_root / "feishu-cli", repo_root / "bin" / "feishu-cli"]
        executables = [path for path in candidates if path.is_file() and os.access(path, os.X_OK)]
        if executables:
            return str(max(executables, key=lambda path: path.stat().st_mtime_ns))
...
def resolve_with_user_info(cli: str, open_ids: list[str], user_token: str | None) -> dict[str, str]:
    ...
    ``user info`` 使用 Tenant Token，且不支持 ``--user-access-token``。脚本级显式
    User Token 只传给消息读取命令，不能透传到这里。
    """
    out: dict[str, str] = {}
    for oid in open_ids:
