...
- [节点类型总览](#节点类型总览)
- [形状节点](#composite_shape形状节点)
- [连接线](#connector连接线)
- [SVG 节点](#svg矢量节点)
- [文本和背景](#纯文本节点)
- [尺寸和位置](#尺寸和位置最佳实践)
- [z_index](#z_index-分层规则)
- [最小示例](#最小示例2-个节点--1-条连线)
...
> 口径说明：本表 600 是 `board lint` 的不扣分阈值（>600 触发 over_capacity 固定扣 0.2）；实测渲染容量上限约 2000 节点，见 `svg-workflow.md` 的实战边界。两个数字不矛盾，一个是质检阈值、一个是容量上限。
