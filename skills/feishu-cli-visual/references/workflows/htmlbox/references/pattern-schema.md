...
- `id` 必填，唯一。
- `x`、`y` 必填，节点左上角坐标（参考坐标系 `900 x 540`；最终 viewBox 会按矩形、`store` 圆和二次 Bézier 曲线的实际包围盒 + padding 自动收紧，无需把节点手动铺满整个画布）。
- `label` 必填。
- `w` 可选，默认 `140`。
- `h` 可选，默认 `54`。
- `sub` 可选，小号大写副标题。
...
- `caption` 必填。仅允许无属性的内联 `<b>` 和 `<code>`；带属性的标签以及其他 HTML 会按普通文本显示，不会执行事件或脚本。
...
- 固定浅色主题。
- 默认自动播放。
- 系统开启“减少动态效果”时不自动播放，不生成飞行 token，也不运行脉冲动画；仍可用上一步/下一步阅读。
