...
var HERE = __dirname; // .../skills/feishu-cli-visual/references/workflows/dataviz/scripts
...
function walk(dir, out) {
  fs.readdirSync(dir, { withFileTypes: true }).forEach(function (ent) {
    ...
    var relativeParts = path.relative(SKILLS_ROOT, p).split(path.sep);
    if (ent.isDirectory() && relativeParts.length === 1 && /-workspace$/.test(ent.name)) return;
