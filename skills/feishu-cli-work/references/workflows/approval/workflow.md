...
**生命周期**：`approval get` → 提交 form 触发 `instance create` → 节点用户 `task approve/reject` → 发起人可中途 `instance cancel` 或 `instance cc` 抄送他人。
...
动态指定节点审批人或抄送人，JSON 形如 `[{"node_id":"n1","value":["ou_xxx"]}]`。
两者都支持对应的 `--*-file` 文件形式，并分别与字符串形式互斥。节点 ID 必须来自审批定义，不能用审批人姓名替代。
...
# 2. 查审批定义拿 widget 结构
feishu-cli approval get 7AB12C... --output raw-json --with-admin-id --with-option | jq '.form'
...
| 审批结果通知到群（消息卡片） | `instance create --open-chat-id <chat>` 内置；或 `feishu-cli-messaging` 发自定义卡片 |
| 给审批文档评论 / 加权限 | `feishu-cli-storage` / 评论命令；审批本身不挂在云文档体系 |

## 相关 skill

- `/feishu-cli-platform` — OAuth 登录、scope 预检、token 状态
- `/feishu-cli-work` — 综合查询入口（也有 approval get / task query 速查段）
