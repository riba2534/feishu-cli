...
无参数命令从 `scripts/palette.json` 复验 categorical、circular、all-pairs、HTMLBox
深蓝画布和 light/dark ordinal 共九个定稿组合。任何 categorical 模式都会无条件拒绝重复 hex；饼图/环图使用
`--pairs circular`，把末项与首项也纳入色盲区分度检查。
