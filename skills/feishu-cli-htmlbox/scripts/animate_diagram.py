...
<body>
  <div class="viz">
    ...
    <div class="canvas-wrap">
      ...
      <svg class="diagram" id="svg" viewBox="__VIEWBOX__" preserveAspectRatio="xMidYMid meet">
        <defs>
          <marker id="arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto">
            <path class="arrow-head" d="M0,0 L10,5 L0,10 z"></path>
          </marker>
        </defs>
      </svg>
    </div>
...
(function () {
  ...
  function spawnTokens(pathEl, gBubbles, duration, reverse) {
    ...
    var handle = { id: null, tokens: tokens, done: false };
    activeRAFs.push(handle);
    ...
    function finish() {
      ...
      handle.done = true;
      handle.id = null;
      tokens.forEach(function (tk) { tk.remove(); });
      var idx = activeRAFs.indexOf(handle);
      if (idx >= 0) activeRAFs.splice(idx, 1);
    }
    function tick(now) {
      if (handle.done) return;
  ...
  Object.keys(PATTERN.edges || {}).forEach(function (eid) {
    ...
    if (edge.label) requestAnimationFrame(function () {
      ...
      var padX = 6, padY = 2, fontSize = 10.5;
      // CJK 全角字符约 10.5px、ASCII 约 6.2px，按字符加权估算标签背景宽度
      var w = edge.label.split("").reduce(function (sum, ch) {
        return sum + (ch.charCodeAt(0) > 0x2e80 ? 10.5 : 6.2);
      }, 0) + padX * 2;
  ...
  function clearActiveRAFs() {
    activeRAFs.forEach(function (h) {
      h.done = true;
      if (h.id != null) cancelAnimationFrame(h.id);
      h.tokens.forEach(function (tk) { tk.remove(); });
    });
    activeRAFs = [];
  }
...
def validate_pattern(pattern: dict) -> None:
    if not isinstance(pattern, dict):
        raise ValueError(f"pattern must be a JSON object, got {type(pattern).__name__}")
    required = ["title", "nodes", "edges", "timeline"]
    missing = [key for key in required if key not in pattern]
    if missing:
        raise ValueError(f"missing required field(s): {', '.join(missing)}")

    if not isinstance(pattern["nodes"], list):
        raise ValueError("field 'nodes' must be a JSON array")
    if not isinstance(pattern["edges"], dict):
        raise ValueError("field 'edges' must be a JSON object")
    if not isinstance(pattern["timeline"], list):
        raise ValueError("field 'timeline' must be a JSON array")
...
def render(pattern: dict) -> str:
    ...
    viewbox = compute_viewbox(pattern)
    vb_parts = viewbox.split()
    aspect = f"{vb_parts[2]} / {vb_parts[3]}" if len(vb_parts) == 4 else "900 / 540"
    return (
        HTML_TEMPLATE
        .replace("__TITLE__", title.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;"))
        .replace("__VIEWBOX__", viewbox)
        .replace("__ASPECT_RATIO__", aspect)
