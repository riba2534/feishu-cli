...
**配色约定**（色值出处：`feishu-cli-dataviz` 统一色板，本管线 surface 为深蓝画布 `#0f1729`）：

- **数据系列色**按固定顺序截取，不循环、不自创：
  `#3370ff, #bf8600, #00ad96, #ea6a03, #7f3bf5, #f54a45, #f04aa8, #2ea121`
  （已在 `#0f1729` 上全项通过校验：色盲最差相邻 ΔE 52.9，对比度全 ≥ 3:1）。
  **单系列例外**：深蓝画布与 slot 1 蓝同色相，单系列主色用 `#00ad96`（青）；多系列才从 slot 1 起按序。
...
- **原样使用上述已校验色值时无需重跑校验**；只有改色值/换底色时复验（校验器随 `feishu-cli-dataviz` 技能分发，先定位再调用，任意 CWD 可用）：

  ```bash
  VP=~/.claude/skills/feishu-cli-dataviz/scripts/validate_palette.js
  [ -f "$VP" ] || VP=skills/feishu-cli-dataviz/scripts/validate_palette.js   # 仓库内开发时（CWD=仓库根）
  node "$VP" "<你的色>" --mode dark --surface "#0f1729"
  ```

  形式选择（画柱还是线还是大数字）与反模式清单见 `feishu-cli-dataviz` 技能。
...
OPT = { backgroundColor:'#0f1729', animationDuration:1400,
  ...
  series:[{type:'line',smooth:true,data:[820,932,901,1134,1290,1330,1320],
    lineStyle:{width:3,color:'#00ad96'},showSymbol:false,
    areaStyle:{color:{type:'linear',x:0,y:0,x2:0,y2:1,colorStops:[{offset:0,color:'rgba(0,173,150,.45)'},{offset:1,color:'rgba(0,173,150,0)'}]}}}]
};
```
...
OPT = { backgroundColor:'#0f1729', animationDuration:1200,
  ...
  series:[
    {name:'新增',type:'bar',stack:'总量',data:[120,200,150,180],itemStyle:{color:'#3370ff',borderRadius:[0,0,0,0]}},
    {name:'留存',type:'bar',stack:'总量',data:[80,120,110,140],itemStyle:{color:'#bf8600'}},
    {name:'付费',type:'bar',stack:'总量',data:[40,60,70,90],itemStyle:{color:'#00ad96',borderRadius:[4,4,0,0]}}]
};
```
...
OPT = { backgroundColor:'#0f1729',
  ...
  series:[{type:'bar',realtimeSort:true,label:{show:true,position:'right',color:'#fff',valueAnimation:true},
    itemStyle:{color:'#00ad96'},data:[10,20,30,40,50,60]}],
...
OPT = { backgroundColor:'#0f1729', animationDuration:1200,
  ...
  series:[{type:'pie',roseType:'area',radius:[24,150],center:['50%','55%'],
    ...
    label:{color:'#bcd2f0'},
    color:['#3370ff','#bf8600','#00ad96','#ea6a03','#7f3bf5','#f54a45'],
...
OPT = { backgroundColor:'#0f1729', animationDuration:1200,
  ...
  series:[{type:'radar',data:[{value:[86,90,75,60,95],areaStyle:{color:'rgba(0,173,150,.35)'},lineStyle:{color:'#00ad96'}}]}]
};
```
...
OPT = { backgroundColor:'#0f1729',
  ...
  series:[{type:'effectScatter',rippleEffect:{brushType:'stroke',scale:3.2},
    ...
    itemStyle:{color:'#00ad96',shadowBlur:12,shadowColor:'rgba(0,173,150,.7)'},
...
OPT = { backgroundColor:'#0f1729',
  ...
  visualMap:{min:0,max:100,calculable:true,orient:'horizontal',left:'center',bottom:10,inRange:{color:['#1c3672','#2b57bd','#4b80f5','#8ab1ff','#cedfff']},textStyle:{color:'#9fb6d6'}},
...
OPT = { backgroundColor:'#0f1729',
  ...
  tooltip:{backgroundColor:'rgba(16,26,46,.92)',textStyle:{color:'#e6edf7'}},
  visualMap:{min:0,max:30,orient:'horizontal',left:'center',bottom:6,inRange:{color:['#1c3672','#2b57bd','#4b80f5','#8ab1ff','#cedfff']},textStyle:{color:'#9fb6d6'}},
...
- **K线**：`series:[{type:'candlestick',data:[[open,close,low,high],...],itemStyle:{color:'#f54a45',color0:'#00ad96',borderColor:'#f54a45',borderColor0:'#00ad96'}}]`，`xAxis:{type:'category'}`。
- **平行坐标**：顶层 `parallelAxis:[{dim:0,name:'性能'},{dim:1,name:'成本'},...]` + `series:[{type:'parallel',data:[[...],[...]],lineStyle:{color:'#00ad96',opacity:.5}}]`。
...
### 漏斗 funnel
漏斗阶段是**有序档位** → 用单色蓝 ramp（浅→深随转化推进），不用多色系列色。
```js
OPT = { backgroundColor:'#0f1729', animationDuration:1200,
  ...
  series:[{type:'funnel',top:50,bottom:20,gap:3,label:{color:'#fff'},
    color:['#a0c0ff','#73a1ff','#4b80f5','#3364d2','#244ca5'],
...
- 水球：`<script src="https://cdn.jsdelivr.net/npm/echarts-liquidfill@3/dist/echarts-liquidfill.min.js"></script>`，`series:[{type:'liquidFill',data:[0.62,0.55,0.5],color:['#00ad96'],backgroundStyle:{color:'#0c1c33'},label:{color:'#fff',fontSize:36}}]`
- 词云：`<script src="https://cdn.jsdelivr.net/npm/echarts-wordcloud@2/dist/echarts-wordcloud.min.js"></script>`，`series:[{type:'wordCloud',gridSize:8,sizeRange:[12,48],rotationRange:[-45,45],textStyle:{color:function(){return ['#3370ff','#bf8600','#00ad96','#ea6a03'][Math.floor(Math.random()*4)]}},data:[{name:'飞书',value:100},{name:'文档',value:80},...]}]`
...
OPT = { backgroundColor:'#0f1729',
  ...
  series:[{type:'graph',layout:'force',roam:true,draggable:true,
    ...
    itemStyle:{color:'#00ad96'},
    data:[{name:'网关',symbolSize:50,itemStyle:{color:'#bf8600'}},{name:'订单',symbolSize:38},{name:'支付',symbolSize:34},{name:'库存',symbolSize:30},{name:'用户',symbolSize:30}],
...
OPT = { backgroundColor:'#0f1729', series:[{type:'tree',data:[TREE],top:30,left:60,right:60,bottom:20,
  ...
  lineStyle:{color:'#2b4a7a'},itemStyle:{color:'#00ad96'},expandAndCollapse:true,initialTreeDepth:3}] };
...
.card{background:#13294a;border-radius:8px;padding:8px 10px;margin:6px 0;font-size:12px;border-left:3px solid #00ad96;
  animation:flow 4s ease-in-out infinite}
@keyframes flow{0%,100%{transform:translateX(0);opacity:.85}50%{transform:translateX(4px);opacity:1;box-shadow:0 0 12px rgba(0,173,150,.4)}}
</style></head><body>
<div class="board">
  <div class="col"><h4>待办</h4><div class="card">需求评审</div><div class="card" style="animation-delay:.5s">接口设计</div></div>
  <div class="col"><h4>进行中</h4><div class="card" style="border-color:#bf8600;animation-delay:1s">开发联调</div></div>
  <div class="col"><h4>已完成</h4><div class="card" style="border-color:#2ea121;animation-delay:1.5s">单元测试</div></div>
</div></body></html>
```
...
@keyframes hue{0%{background:#bf8600}50%{background:#00ad96}100%{background:#3370ff}}
.box{width:60px;height:60px;border-radius:12px;background:#f54a45;animation:spin 2.5s linear infinite}
.dot{width:56px;height:56px;border-radius:50%;background:#00ad96;animation:pulse 1.8s ease-in-out infinite}
...
// loop 内：
x.fillStyle='#00ad96';P.forEach(function(p){p.x=(p.x+p.vx+cv.width)%cv.width;p.y=(p.y+p.vy+cv.height)%cv.height;x.beginPath();x.arc(p.x,p.y,2,0,7);x.fill()});
x.strokeStyle='rgba(0,173,150,.15)';P.forEach(function(a){P.forEach(function(b){var d=Math.hypot(a.x-b.x,a.y-b.y);if(d<90){x.beginPath();x.moveTo(a.x,a.y);x.lineTo(b.x,b.y);x.stroke()}})});
```
...
<linearGradient id="g" x1="0" x2="1"><stop offset="0" stop-color="#3370ff"/><stop offset=".5" stop-color="#00ad96"/><stop offset="1" stop-color="#bf8600"/></linearGradient></defs>
<path class="wave" stroke="url(#g)" d="M40,210 C120,210 130,90 210,110 C290,130 290,250 370,230 C450,210 450,80 520,110"/>
<circle class="pulse" cx="210" cy="110" r="4" fill="#ea6a03"/><circle class="pulse" cx="370" cy="230" r="4" fill="#bf8600" style="animation-delay:.6s"/>
</svg></body></html>
```
...
<circle class="c" cx="195" cy="140" r="100" fill="#3370ff"/><circle class="c" cx="285" cy="140" r="100" fill="#00ad96" style="animation-delay:1s"/>
<circle class="c" cx="240" cy="215" r="100" fill="#bf8600" style="animation-delay:2s"/>
...
ECharts 骨架 + `series:[{type:'pictorialBar',symbol:'rect',symbolRepeat:true,symbolSize:[12,4],symbolMargin:2,data:[40,60,55,80],itemStyle:{color:'#00ad96'}}]`，配 `xAxis:{type:'category',data:[...]}` + `yAxis`.
...
.up{color:#00ad96;font-size:12px}.down{color:#f54a45;font-size:12px}
