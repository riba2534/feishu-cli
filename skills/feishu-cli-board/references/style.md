...
> **色值唯一出处**：`feishu-cli-dataviz` 技能的 `references/palette.md`（统一色板，
> 已通过色盲区分度/对比度脚本校验）。本文件只讲画板场景的**取用方式**与主题变体，
> 不自创色值。改色后用 `feishu-cli-dataviz` 的 `scripts/validate_palette.js` 复验。
...
**原样按序取用时无需校验**（该色板已预校验，结论见 `feishu-cli-dataviz` 的 palette.md）。
...
文字旁的色块/图例承载。形式选择与反模式清单见 `feishu-cli-dataviz` 技能。
