...
6. **分栏布局（Grid）**：识别 `<div style="display: flex">` HTML 结构，自动创建飞书分栏块，支持 2-5 列及列宽比例
7. **API 限流处理**：自动重试，避免 429 错误
8. **并发控制**：图表和表格分别使用独立的 worker 池（默认图表 5、表格 3 并发）
...
- **分栏布局**（`<div style="display: flex">` HTML 结构 → 飞书 Grid 块，支持 2-5 列，`flex: N` 中 N=1-99 表示列宽百分比）
...
- 未指定 N 或 N=1 时各列等分
- 飞书 Grid 支持 2-5 列，超过 5 列时取前 5 列
- 每列内部支持任意 Markdown 内容（段落、标题、列表、图片、代码块等）
- 导入时自动检测 `<div style="display: flex">` 结构并创建飞书分栏块
