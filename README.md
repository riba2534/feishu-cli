...
`skills/` 目录包含 **29 个** 为 [Claude Code](https://claude.ai/claude-code) 设计的技能文件，让 AI Agent 能够直接操作飞书 — 创建文档、发送消息、管理权限，全部通过自然语言驱动。
...
| `feishu-cli-dataviz` | 可视化设计系统（统一色板 + 色盲/对比度校验器 + 图表形式选择启发式） | "画个数据图表"、"做个 dashboard" |
