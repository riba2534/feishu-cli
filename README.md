...
| **Raw API** | `api GET/POST/PUT/DELETE/PATCH <path>` 裸调任意 OpenAPI endpoint（自动鉴权 + 错误码处理 + `--dry-run`/`-o`/`--include-headers`/`--timeout`，并支持 `--jq`/`--format json\|pretty\|table\|ndjson\|csv`） |
...
| `feishu-cli-api` | 裸调任意飞书 OpenAPI endpoint（覆盖未封装接口，支持 --jq/--format） | "调用这个飞书接口"、"raw api" |
